...
WS_FILES = ["app.py", "config.json", "readme.md", "notes.txt", "utils.js"]
EXPECTED_INCLUDED = ["app.py", "utils.js"]
EXPECTED_EXCLUDED = ["config.json", "readme.md", "notes.txt"]
...
def grade():
    ...
    results = []
    p = 0
    must = {"R1": False, "R2": False, "R4": False, "R5": False, "R6": False}
    ...
    r3 = all(isinstance(m, dict) and "file" in m and "file_extension" in m and "line_numbers" in m and "lines" in m for m in matches) if matches else False
    results.append({"rubric_id": "R3", "passed": r3, "score": 1 if r3 else 0, "reason": "OK" if r3 else "missing fields", "evidence": {}})
    if r3: p += 1

    r4 = "app.py" in mfiles
    results.append({"rubric_id": "R4", "passed": r4, "score": 1 if r4 else 0, "reason": "OK" if r4 else "not in matches", "evidence": {"matched": mfiles}})
    if r4: p += 1; must["R4"] = r4

    r5 = "utils.js" in mfiles
    results.append({"rubric_id": "R5", "passed": r5, "score": 1 if r5 else 0, "reason": "OK" if r5 else "not in matches", "evidence": {}})
    if r5: p += 1; must["R5"] = r5

    r6 = all(f not in mfiles for f in EXPECTED_EXCLUDED)
    results.append({"rubric_id": "R6", "passed": r6, "score": 1 if r6 else 0, "reason": "OK" if r6 else "non-code files included", "evidence": {}})
    if r6: p += 1; must["R6"] = r6

    r7 = all(f in WS_FILES for f in mfiles)
    results.append({"rubric_id": "R7", "passed": r7, "score": 1 if r7 else 0, "reason": "OK" if r7 else "invalid files", "evidence": {}})
    if r7: p += 1

    r8 = all(m.get("file_extension") in [".py", ".js", ".txt", ".json", ".md"] for m in matches) if matches else False
    results.append({"rubric_id": "R8", "passed": r8, "score": 1 if r8 else 0, "reason": "OK" if r8 else "bad extensions", "evidence": {}})
    if r8: p += 1

    r9 = all(f in WS_FILES for f in mfiles)
    results.append({"rubric_id": "R9", "passed": r9, "score": 1 if r9 else 0, "reason": "OK" if r9 else "bad files", "evidence": {}})
    if r9: p += 1
